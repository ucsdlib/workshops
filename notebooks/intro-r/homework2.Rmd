...
Create a ggplot function that contains the built in diamonds data and an aesthetics layer.  Map carat on the x axis and price on the y axis.  Assign the ggplot function to an object: **myplot**

Using **+** , add a geom point with an alpha of 0.1, smooth, and gray theme layers to the myplot object.  Map **clarity** to the color argument in the geom_smooth function (add **se=FALSE** to geom_smooth to turn off error shading). 
